#![no_std]
use soroban_sdk::{
    contract, contractclient, contractimpl, contracttype, Env, Address, Bytes, Vec, log,
};
...
impl DocumentTimestampContract {
    pub fn store_document(
        ...
        let mut creator_docs: Vec<Bytes> = env
            .storage()
            .persistent()
            .get(&creator_key)
        ...
        if let Some(receiver_addr) = receiver {
            ...
            let mut receiver_docs: Vec<Bytes> = env
                .storage()
                .persistent()
                .get(&receiver_key)
                .unwrap_or(Vec::new(&env));
    ...
    pub fn sign_document(env: Env, ipfs_cid: Bytes, signer: Address) {
        ...
        let doc_key = DataKey::Document(ipfs_cid.clone());
        let mut metadata: DocumentMetadata = env
            .storage()
            .persistent()
            .get(&doc_key)
